...
class AlbumListAdapter extends BaseAdapter {
    ...
    public AlbumListAdapter(final Context context, final List<GridItem> buckets) {
        this.items = buckets;
    ...
    public View getView(int position, View convertView, final ViewGroup parent) {
        if (items.get(0) instanceof BucketItem) { // show buckets
            ViewHolder holder;
            if (convertView == null) {
                convertView = mInflater.inflate(R.layout.albumb_list_row, null);
                holder = new ViewHolder();
                holder.ivAlbumImage = (ImageView) convertView.findViewById(R.id.ivAlbumImage);
                holder.tvAlbumName = (TextView) convertView.findViewById(R.id.tvAlbumName);
                holder.tvImageCount = (TextView) convertView.findViewById(R.id.tvImageCount);
                convertView.setTag(holder);
            } else {
                holder = (ViewHolder) convertView.getTag();
            }
            BucketItem bi = (BucketItem) items.get(position);
            holder.tvAlbumName.setText(bi.images > 1 ?
                    bi.name  :
                    bi.name);
            holder.tvImageCount.setText(bi.images > 1 ?
                    context.getString(R.string.images, bi.images) :
                    bi.name);
            ImageLoader.getInstance().displayImage("file://" + bi.path, holder.ivAlbumImage);
            return convertView;
    ...
    private static class ViewHolder {
        ...
        private TextView tvAlbumName, tvImageCount;
    }
